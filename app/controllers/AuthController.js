        ...
        try {
            ...
            const mailInfo = {
                ...
                html: `
                    ...
                    <a style="padding:10px 20px;background-color:#ea1e30;text-decoration:none;color:#fffffe;border-radius:5px;display:inline-block;max-width:70%;font-size:16px;margin:10px 0" href="http://${process.env.CLIENT_SERVER}/forgotPassword/${token}">Nhận mật khẩu mới</a>
                    <p>Cảm ơn và hẹn gặp lại bạn sau.</p>
                    <p style="color:#ea1e30">Chú ý: Email này chỉ có hiệu lực trong 10 phút kể từ khi email được gửi đến.</p>`
            }
            await sendMail(mailInfo);
